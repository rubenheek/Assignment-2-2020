    ...
    for x in range(0, len(releases)):
        ...
        for y in range(0, len(releases)):
            if x <= y:
                ...
                command = f"jsinspect -I -L -r pmd '/usr/jquery-data/{releases[x]}/src' '/usr/jquery-data/{releases[y]}/src'"
                ...
                command = f"cloc --ignore-whitespace {releases[x]} --xml"
                result = subprocess.getoutput(command)
                ...
                command = f"cloc --ignore-whitespace {releases[y]} --xml"
                result = subprocess.getoutput(command)
                found = re.findall(r'blank="[+]?\d+', result)
                filtered_blank = re.search("\d+", found[0]).group(0)
                found = re.findall(r'comment="[+]?\d+', result)
